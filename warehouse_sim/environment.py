...
class WarehouseEnv(gym.Env):
    ...
    def __init__(self, warehouse: Warehouse, max_steps: int = 500):
        ...
        obs_size = (
            400 +  # shelf item counts (400 shelves)
            len(warehouse.robots) * 4 +  # robot states (position x, y, mode, availability)
            50 +  # item availability (50 item types)
            10   # additional metrics (pending orders, average delay, etc.)
        )
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        for robot in self.warehouse.robots:
            ...
            obs_parts.extend([
                x, y, robot.mode, 
                1.0 if robot.available else 0.0
            ])
        ...
        obs_parts.extend([
            ...
            len([r for r in self.warehouse.robots if not r.available]),  # busy robots
    ...
    def _calculate_reward(self) -> float:
        ...
        # Reward calculation
        reward = (
            completed_orders * 1.0 +  # Positive for completions
            -avg_delay * 0.1 +  # Penalty for delays
            -pending_orders * 0.05 +  # Penalty for queue buildup
            utilization * 0.5  # Bonus for keeping robots busy
        )
    ...
    def _get_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'robot_utilization': len([r for r in self.warehouse.robots if not r.available]) / len(self.warehouse.robots),
            'buffer_status': [
                {
                    'station_id': i,
