...
class WarehouseExperiment:
    ...
    def run_single_simulation(self, 
                             ...
                             buffer_enabled: bool, 
                             robot_count: int,
                             buffer_size: int) -> SimulationMetrics:
        """Run a single simulation with specified parameters."""
        
        logger.info(f"Running simulation: seed={seed}, buffer={buffer_enabled}, "
        ...
        for step in range(self.config.simulation_steps):
            ...
            if step % 100 == 0 and step > 0:
                ...
                logger.debug(f"Step {step}: {len(warehouse.order_compleated)} completed, "
                           f"avg delay: {avg_delay:.2f}")
        
        # Calculate final metrics
        metrics = self._calculate_metrics(
            warehouse, seed, buffer_enabled, robot_count, buffer_size,
            buffer_hits, robot_busy_time, robot_total_distance
        )
    ...
    def _calculate_metrics(self, 
                          ...
                          robot_total_distance: Dict[int, float]) -> SimulationMetrics:
        ...
        completion_times = [order.done_time for order in completed_orders if order.done_time is not None]
        ...
        avg_robot_distance = (sum(robot_total_distance.values()) / 
                             len(robot_total_distance) if robot_total_distance else 0)
            ...
            for buffer_enabled in self.config.buffer_enabled_scenarios:
                for robot_count in self.config.robot_counts:
                    for buffer_size in self.config.buffer_sizes:
                        ...
                        if not buffer_enabled and buffer_size != self.config.buffer_sizes[0]:
                            continue
...
class ExperimentAnalyzer:
    ...
    def analyze_buffer_impact(self):
        ...
        for metric in metrics_to_compare:
            ...
            pooled_std = np.sqrt(((np.std(enabled_values)**2 + np.std(disabled_values)**2) / 2))
    ...
    def analyze_delay_distributions(self):
        ...
        fig.suptitle('Order Delay Distribution Analysis', fontsize=16, fontweight='bold')
        ...
        for i, (label, group) in enumerate(buffer_groups.items()):
            ...
            if all_delays:
                ax1.hist(all_delays, bins=30, alpha=0.7, label=label, 
        ...
        for buffer_enabled in [True, False]:
            for robot_count in sorted(set(r.robot_count for r in self.results)):
                group_delays = [r.mean_delay for r in self.results 
                              if r.buffer_enabled == buffer_enabled and r.robot_count == robot_count]
                if group_delays:
                    delay_data.append(group_delays)
                    labels.append(f"{'Buffer' if buffer_enabled else 'No Buffer'}\n{robot_count} robots")
        
        if delay_data:
            bp = ax2.boxplot(delay_data, labels=labels, patch_artist=True)
            
            # Color boxes
            for i, patch in enumerate(bp['boxes']):
                color = colors[0] if 'Buffer' in labels[i] and 'No Buffer' not in labels[i] else colors[1]
        ...
        for buffer_enabled in [True, False]:
            group_throughput = [r.throughput for r in self.results if r.buffer_enabled == buffer_enabled]
            if group_throughput:
                throughput_data.append(group_throughput)
                throughput_labels.append('Buffer Enabled' if buffer_enabled else 'Buffer Disabled')
        
        if throughput_data:
            bp = ax3.boxplot(throughput_data, labels=throughput_labels, patch_artist=True)
            for i, patch in enumerate(bp['boxes']):
        ...
        for buffer_enabled in [True, False]:
            group_variance = [r.variance_delay for r in self.results if r.buffer_enabled == buffer_enabled]
            if group_variance:
                variance_data.append(group_variance)
                variance_labels.append('Buffer Enabled' if buffer_enabled else 'Buffer Disabled')
        
        if variance_data:
            bp = ax4.boxplot(variance_data, labels=variance_labels, patch_artist=True)
            for i, patch in enumerate(bp['boxes']):
                patch.set_facecolor(colors[i])
                patch.set_alpha(0.7)
    ...
    def analyze_robot_buffer_variations(self):
        ...
        buffer_sizes = sorted(set(r.buffer_size for r in self.results if r.buffer_enabled))
        ...
        metric_titles = ['Mean Delay', 'Throughput', 'Robot Utilization', 'Buffer Hit Rate']
        
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Performance Heatmaps: Robot Count vs Buffer Size', fontsize=16, fontweight='bold')
            ...
            for i, robot_count in enumerate(robot_counts):
                for j, buffer_size in enumerate(buffer_sizes):
                    ...
                    matching_results = [
                        ...
                        if (r.robot_count == robot_count and 
                            r.buffer_size == buffer_size and 
                            r.buffer_enabled)
                    ]
    ...
    def _plot_buffer_comparison(self, buffer_enabled: List, buffer_disabled: List):
        ...
        fig.suptitle('Buffer Enabled vs Disabled Performance Comparison', 
        ...
        for idx, (metric, ylabel, preference) in enumerate(metrics_info):
            ...
            if metric == 'buffer_hit_rate':
                ...
                disabled_values = [0] * len(buffer_disabled)  # No buffer hits when disabled
            ...
            if len(enabled_values) > 1 and len(disabled_values) > 1:
                t_stat, p_value = stats.ttest_ind(enabled_values, disabled_values)
                if p_value < 0.05:
                    ax.text(0.5, 0.95, f'p < 0.05*', transform=ax.transAxes, 
    ...
    def generate_summary_report(self):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"Robot Counts Tested: {sorted(set(r.robot_count for r in self.results))}\n")
            f.write(f"Buffer Sizes Tested: {sorted(set(r.buffer_size for r in self.results))}\n")
            f.write(f"Simulation Steps per Run: {self.results[0].simulation_steps if self.results else 'N/A'}\n\n")
            ...
            if buffer_enabled and buffer_disabled:
                ...
                for metric in metrics:
                    ...
                    disabled_avg = np.mean([getattr(r, metric) for r in buffer_disabled])
                    improvement = ((enabled_avg - disabled_avg) / disabled_avg * 100 
                                 if disabled_avg != 0 else 0)
                ...
                overall_hit_rate = total_buffer_hits / total_completed * 100 if total_completed > 0 else 0
            ...
            f.write(f"  Configuration: {best_delay.robot_count} robots, "
            ...
            f.write(f"Maximum Throughput: {best_throughput.throughput:.4f} orders/timestep\n")
            f.write(f"  Configuration: {best_throughput.robot_count} robots, "
...
def main():
    ...
    config = ExperimentConfig(
        seeds=[42, 123, 456, 789, 999],  # 5 random seeds for statistical robustness
        simulation_steps=1000,  # Sufficient steps for meaningful results
        robot_counts=[3, 4, 5, 6, 7],  # Vary robot count to see scaling effects
        buffer_sizes=[4, 6, 8, 10, 12],  # Different buffer sizes
        buffer_enabled_scenarios=[True, False],  # Compare with and without buffers
    ...
    if buffer_enabled and buffer_disabled:
        ...
        delay_improvement = ((disabled_delay - enabled_delay) / disabled_delay * 100 
                           if disabled_delay > 0 else 0)
        ...
        throughput_improvement = ((enabled_throughput - disabled_throughput) / disabled_throughput * 100 
                                if disabled_throughput > 0 else 0)
        
        print(f"Average delay improvement with buffers: {delay_improvement:+.1f}%")
        print(f"Average throughput improvement with buffers: {throughput_improvement:+.1f}%")
        
        total_buffer_hits = sum(r.buffer_hits for r in buffer_enabled)
        total_completed = sum(r.total_orders_completed for r in buffer_enabled)
        hit_rate = total_buffer_hits / total_completed * 100 if total_completed > 0 else 0
